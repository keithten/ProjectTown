...
class SNumericEntryBoxFix : public SCompoundWidget
{
public:
...
public:
	...
	SLATE_BEGIN_ARGS( SNumericEntryBoxFix<NumericType> )	//APPARANCE: class renamed
		...
		, _UndeterminedString( SNumericEntryBoxFix<NumericType>::DefaultUndeterminedString ) //APPARANCE: class renamed
		, _AllowSpin( false )
	...
	SLATE_END_ARGS()
	SNumericEntryBoxFix() //APPARANCE: class renamed
	{
	}
	...
	void Construct( const FArguments& InArgs )
	{
		...
		{
			Interface->GetOnSettingChanged()->AddSP(this, &SNumericEntryBoxFix::ResetCachedValueString);
		}
		...
		if(bDisplayToggle)
		{
			...
			.OnCheckStateChanged( this, &SNumericEntryBoxFix::HandleToggleCheckBoxChanged, InArgs._OnToggleChanged );	//APPARANCE: class renamed
		}
		...
		if(bAllowSpin)
		{
			SAssignNew( SpinBox, SSpinBox<NumericType> )
				.PreventThrottling( true )	//APPARANCE: Fix for issue of uninitialised SSpinBox member
				.Style( InArgs._SpinBoxStyle )
				.Font(InArgs._Font.IsSet() ? InArgs._Font : InArgs._EditableTextBoxStyle->TextStyle.Font)
				.Value( this, &SNumericEntryBoxFix<NumericType>::OnGetValueForSpinBox ) //APPARANCE: class renamed
				.Delta( InArgs._Delta )
				...
				.ToolTipText(this, &SNumericEntryBoxFix<NumericType>::GetValueAsText);
		}

		// Always create an editable text box.  In the case of an undetermined value being passed in, we cant use the spinbox.
		SAssignNew( EditableText, SEditableText )
			.Text( this, &SNumericEntryBoxFix<NumericType>::OnGetValueForTextBox ) //APPARANCE: class renamed
			.ToolTipText(this, &SNumericEntryBoxFix<NumericType>::GetValueAsText)
			...
			.OnTextChanged( this, &SNumericEntryBoxFix<NumericType>::OnTextChanged ) //APPARANCE: class renamed
			.OnTextCommitted( this, &SNumericEntryBoxFix<NumericType>::OnTextCommitted ) //APPARANCE: class renamed
		...
		else
		{
			MainContents =
				SNew( SBorder )
				.BorderImage( this, &SNumericEntryBoxFix<NumericType>::GetBorderImage ) //APPARANCE: class renamed
...
template <typename NumericType>
const FLinearColor SNumericEntryBoxFix<NumericType>::RedLabelBackgroundColor( 0.594f, 0.0197f, 0.0f ); //APPARANCE: class renamed

template <typename NumericType>
const FLinearColor SNumericEntryBoxFix<NumericType>::GreenLabelBackgroundColor( 0.1349f, 0.3959f, 0.0f ); //APPARANCE: class renamed

template <typename NumericType>
const FLinearColor SNumericEntryBoxFix<NumericType>::BlueLabelBackgroundColor( 0.0251f, 0.207f, 0.85f ); //APPARANCE: class renamed

template <typename NumericType>
const FLinearColor SNumericEntryBoxFix<NumericType>::LilacLabelBackgroundColor( 0.8f, 0.121f, 0.8f ); //APPARANCE: class renamed

template <typename NumericType>
const FText SNumericEntryBoxFix<NumericType>::DefaultUndeterminedString = FText::FromString( TEXT( "---" ) ); //APPARANCE: class renamed

template<typename NumericType>
const int32 SNumericEntryBoxFix<NumericType>::DefaultMinFractionalDigits = 1;

template<typename NumericType>
const int32 SNumericEntryBoxFix<NumericType>::DefaultMaxFractionalDigits = 6;