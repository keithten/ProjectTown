...
//APPARANCE: use fixed version of numeric entry box
//#include "Widgets/Input/SNumericEntryBox.h"
#include "SNumericEntryBoxFix.h"	//local copy to fix a bug
//...APPARANCE: use fixed version of numeric entry box
...
class SNumericVectorInputBoxFix : public SCompoundWidget	//APPARANCE: class renamed
{
public:
...
private:
	using ThisClass = SNumericVectorInputBoxFix<NumericType, VectorType, NumberOfComponents>; 	//APPARANCE: class renamed
...
public:
	//SLATE_BEGIN_ARGS(SNumericVectorInputBoxFix<NumericType>)	//APPARANCE: class renamed
...
private:
		...
		const FMenuExtensionDelegate& OnContextMenuExtenderComponent)
	{
		...
		{
			LabelWidget = SNumericEntryBoxFix<NumericType>::BuildNarrowColorLabel(LabelColor); 	//APPARANCE: use fixed version
		}
		...
		[
			SNew(SNumericEntryBoxFix<NumericType>) 	//APPARANCE: use fixed version
			...
			.LabelLocation(SNumericEntryBoxFix<NumericType>::ELabelLocation::Inside) 	//APPARANCE: use fixed version
			.Label()
			[
				LabelWidget
			]
	...
	void ConstructX(const FArguments& InArgs, TSharedRef<SHorizontalBox> HorizontalBox)
	{
		ConstructComponent(0,
			InArgs,
			SNumericEntryBoxFix<NumericType>::RedLabelBackgroundColor, 	//APPARANCE: use fixed version
	...
	void ConstructY(const FArguments& InArgs, TSharedRef<SHorizontalBox> HorizontalBox)
	{
		ConstructComponent(1,
			InArgs,
			SNumericEntryBoxFix<NumericType>::GreenLabelBackgroundColor, 	//APPARANCE: use fixed version
	...
	void ConstructZ(const FArguments& InArgs, TSharedRef<SHorizontalBox> HorizontalBox)
	{
		ConstructComponent(2,
			InArgs,
			SNumericEntryBoxFix<NumericType>::BlueLabelBackgroundColor, 	//APPARANCE: use fixed version
	...
	void ConstructW(const FArguments& InArgs, TSharedRef<SHorizontalBox> HorizontalBox)
	{
		ConstructComponent(3,
			InArgs,
			SNumericEntryBoxFix<NumericType>::LilacLabelBackgroundColor, 	//APPARANCE: use fixed version
 ...
 */
using SVectorInputBoxFix = SNumericVectorInputBoxFix<float, UE::Math::TVector<float>, 3>;	//APPARANCE: not needed with class renamed
